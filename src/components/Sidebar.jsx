...
export default function Sidebar() {
  ...
  return (
    <div
      className={` ${isOpen ? "right-0" : "-right-full"} w-full bg-white
     fixed top-0 h-full shadow-2xl md:w-[35vw] xl:max-w-[30vw] trasition-all duration-300 z-20 px-4 lg:px[35px]`}
    >
      ...
      <div className=" flex flex-col gap-y-2 h-[520px] lg:h-[640px] overflow-y-auto overflow-x-hidden border-b">
        {cart.map((item) => {
        return  <CartItem amount={item.amount} title={item.title} key={item.id} id={item.id} img={item.image} price={item.price} />
      } )}
      </div> 
